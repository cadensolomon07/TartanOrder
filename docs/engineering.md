...
Those were synthetic typed utterances through real HTTP and Gemini, not human microphone trials or an accuracy benchmark. A fresh Chromium context with browser networking disabled also completed Local only ordering on the prebuilt local production server: $13.50 receipt, zero interpret HTTP requests and zero page errors. This was browser-offline simulation; the physical Wi-Fi-off check in the [runbook](runbook.md) is historical V1 evidence. These offline checks are separate from the live deployment verification below.
