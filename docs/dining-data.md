...
The campus selector retains **eleven locations in the user's requested order**, with **237 priced configurations across eight locations** and **50 unavailable menu previews**. This order is a requested shortlist, not a measured popularity ranking. TartanOrder remains a simulation with no purchase, payment, POS call, or kitchen dispatch.

The complete source archive retains **45 dining-directory entries and 495 campus configurations**. Of those configurations, 258 belong to locations outside the public shortlist and remain only for historical provenance and internal regression/replay fixtures. Archived campus locations remain unavailable through the public controller. The meal/dietary milestone adds a separate, explicit **Demo Counter · fictional recipes** entry; it does not replace or expand the eleven-campus-venue shortlist. The archive does not imply that any location is currently open or operating.

## Meal/dietary evidence boundary

The implemented shared menu version is **`cmu-meal-2026-09-12`**. Before using the public page, verify that `/api/health` reports this version and consult [integration.md](integration.md) for the executed public result. Campus counts below are unchanged; the separate Demo Counter adds eleven fictional items, not eleven sourced campus configurations. Its prices are illustrative and its recipes are not CMU recipes.

[src/contracts/food.ts](../src/contracts/food.ts) is the authoritative recipe/evidence source used by the shared compatibility check. Each record distinguishes ingredient completeness, declared allergens and coverage, preparation/cross-contact status, source explanation and verification date. Fictional recipe definitions are dated **2026-09-12**. Campus ingredient/preparation verification dates are **null**, because the catalog has not validated complete evidence; menu-price provenance does not substitute for it. Published dietary terms or a named gluten-free wrap alone never make an item an allergy match.

The fictional Veggie Wrap uses plant-based mayo; with Fries and Water it gives a **$12 vegan example**. Extra cheese is a supported $1 wrap modifier that introduces a dairy/vegan conflict. The fictional Burger declares sesame, Onion Rings declare peanut, and Side Salad deliberately has unknown preparation evidence. These are test fixtures, not claims about real kitchens. Unknown/custom allergens and ambiguous “nuts” stay explicit. Removing mayo or serving dressing separately does not prove that an allergen is absent elsewhere or remove cross-contact uncertainty.

For campus orders with dietary/allergy requirements, incomplete evidence is displayed as **Needs verification**, remains inspectable and is excluded from automatic matches. Known allergy conflicts and unknown preparation cannot be overridden. The on-page staff summary is a local aid; it contacts nobody and offers no safety guarantee. Dietary preferences, allergies and dislikes remain separate; exact preference exceptions do not waive allergies. See [FARE's cross-contact guidance](https://www.foodallergy.org/resources/avoiding-cross-contact) and [engineering.md](engineering.md).

## Public catalog and provenance

[src/contracts/campus.ts](../src/contracts/campus.ts) remains the released data source. `ACTIVE_LOCATION_IDS` and `ACTIVE_DINING_LOCATIONS` define the eleven-campus-venue order; the public policy separately permits `demo`; `ACTIVE_CAMPUS_ITEMS` selects priced configurations from `CAMPUS_ITEMS`. `UNPRICED_MENU_ITEMS` contains disabled previews, including eight Hunan entrees whose published base prices are known but whose complete side configurations are unresolved.
...
For updates, inspect changed sources visually and resolve sizes, required choices and prices before editing the canonical catalog. Preserve the live-directory baseline separately from supplied menu provenance, record PDF hashes/pages, and bump `MENU_VERSION` when the orderable catalog changes. Run relevant contract, parser, engine and interface checks before release. A missing link or failed source check must not silently delete or reprice the catalog, reactivate archived locations, or turn unavailable previews into orderable items.
