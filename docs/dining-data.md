...
The campus selector lists **eight locations in the user's requested order**, with **237 priced configurations** and the **unavailable menu previews** that belong to them. Three requested venues with no complete prices (Au Bon Pain at Skibo, Capital Grains, Schatz Dining Room) were removed from the selector on September 12, 2026; their directory rows and previews stay archived (`active_rank` null) and are not selectable or orderable. This order is a requested shortlist, not a measured popularity ranking. TartanOrder remains a simulation with no purchase, payment, POS call, or kitchen dispatch.

The complete source archive retains **45 dining-directory entries and 495 campus configurations**. Of those configurations, 258 belong to locations outside the public shortlist and remain only for historical provenance and internal regression/replay fixtures. Archived campus locations remain unavailable through the public controller. The meal/dietary milestone adds a separate, explicit **Demo Counter · fictional recipes** entry; it does not replace or expand the eight-campus-venue shortlist. The archive does not imply that any location is currently open or operating.

## Meal/dietary evidence boundary

The implemented shared menu version is **`cmu-dietary-2026-09-12`**. Before using the public page, verify that `/api/health` reports this version and consult [integration.md](integration.md) for the executed public result. Campus counts below are unchanged; the separate Demo Counter adds eleven fictional items, not eleven sourced campus configurations. Its prices are illustrative and its recipes are not CMU recipes.

Food evidence is catalog data: every item row carries `foodEvidence`, generated by the seed. The fictional Demo Counter recipes come from [src/contracts/food.ts](../src/contracts/food.ts); campus items carry ingredients **inferred** from their published name and description (see the next section). The shared compatibility check reads that stored evidence. Each record distinguishes ingredient completeness, declared allergens and coverage, preparation/cross-contact status, source explanation and verification date. Fictional recipe definitions are dated **2026-09-12**. Campus ingredient/preparation verification dates are **null**, because the catalog has not validated complete evidence; menu-price provenance does not substitute for it. Published dietary terms or a named gluten-free wrap alone never make an item an allergy match.
...
## Inferred ingredient marks (campus items)

For campus items the seed infers ingredients from the published name and description with a fixed keyword lexicon (meats, fish and shellfish, dairy, egg, wheat, soy, peanut, tree nuts, sesame, and plant cues such as “veggie” or “vegan”). The result is stored on the item with provenance `inferred_campus` and completeness `partial`, and the kiosk labels it “from menu wording” with the note *Ingredient marks are inferred from the published names and descriptions and are not verified; missing ingredients are not confirmed.*

The policy is asymmetric on purpose. A detected ingredient counts against the matching restriction: “Chicken Wings” conflicts with a vegetarian or vegan preference, “Cheddar” yields a milk conflict for a milk allergy. The absence of a keyword proves nothing: allergy checks on campus items still show **Needs verification**, and only a published “veggie”/“vegan” wording (`dietaryClaims`) satisfies a *preference*, never an allergy. Preparation and cross-contact remain unknown for every campus item.

Ingredient removals are inferred the same way. Removable toppings named in the wording (bacon, cheese, onions, lettuce, tomato, pickles, mayo, sauces and the like) become `no_<ingredient>` modifiers with price 0 and an `effect` that removes that ingredient from the evidence; each item lists only the removals that apply to it. A removal changes the marks shown for the cart line but does not establish that the allergen is absent elsewhere in the recipe. Whether a counter honours a removal is not verified.

## Public catalog and provenance

The canonical released data is the active catalog version in the Supabase project TartanOrder: version `cmu-dietary-2026-09-12` (published on September 12, 2026 as the third version, after `cmu-shortlist-2026-09-12` and `cmu-meal-2026-09-12`, adding stored food evidence, inferred ingredient removals and the eight-venue selector) with 506 items (495 campus configurations plus the 11 internal Demo Counter items), 46 locations (45 directory rows plus the internal Demo Counter; the eight public venues carry `active_rank` 1–8 in the requested order), 50 unavailable previews, and the 7 Demo Counter modifiers plus the generated `no_<ingredient>` removals (`modifiers.effect` and `items.food_evidence` are the columns added for this version). The application loads that version at request time and reports it in `/api/health`; published rows cannot be updated or deleted, so a correction is a new version. [src/contracts/campus.ts](../src/contracts/campus.ts) is the generator input for that seed and the labelled `CATALOG_SOURCE=bundled` fallback, not a runtime data source: `ACTIVE_LOCATION_IDS` and `ACTIVE_DINING_LOCATIONS` define the eleven-campus-venue order; the public policy separately permits the fictional `demo` counter for the meal demonstration; `ACTIVE_CAMPUS_ITEMS` selects priced configurations from `CAMPUS_ITEMS`, and `UNPRICED_MENU_ITEMS` contains disabled previews, including eight Hunan entrees whose published base prices are known but whose complete side configurations are unresolved.
...
| Capital Grains (removed from the selector; no priced configurations) | 179 | 0 | 6 | Directory-linked CMU PDF; complete item prices unresolved |
| Au Bon Pain at Skibo Café (removed from the selector; no priced configurations) | 113 | 0 | 30 | Supplied CMU-hosted PDF; no item prices printed |
...
| Schatz Dining Room (removed from the selector; no priced configurations) | 108 | 0 | 6 | Directory-linked web menu; entry prices unverified |
| **Total** | | **237** | **50** | |
