...
Published code commit `85ff5b1` to main; [GitHub CI 34715125016](https://github.com/cadensolomon07/TartanOrder/actions/runs/34715125016) passed all steps. Vercel deployment `dpl_E3ZX1hUwpQxELCTAEYTm97nmpCLE` is ready at https://tartan-order.vercel.app/. Public health retains API 3, Gemini and Supabase catalog/persistence. A fresh logged-out Chromium check completed both synthetic microphone journeys on production: four real Gemini requests, no fallback or page errors, notes retained through review and receipt. [Production evidence](../evals/runs/gemini-audio-production-2026-09-12.json). The first production harness attempt stopped before recording while looking for an optional read-aloud control; the harness was adjusted to tolerate its absence and use the selected-language label. No app change was required. Human microphone verification remains pending.
...
Checks in a fresh integration worktree on pinned Node 22.23.2: `npm ci` succeeded with zero audit vulnerabilities; typecheck and lint passed; the complete unit/contract suite passed 820 tests with 30 opt-in skips. A subsequently added runtime-only item/notes regression passed with all 28 existing parser-note tests. Build and the client-bundle secret check passed; Playwright passed 38 tests with 3 database-dependent skips. This includes both language fixtures, manual notes through receipt, dietary controls, offline rules and actual HTTP 503 fallback handling. These browser checks used the explicitly labelled bundled catalog with persistence off; no live Supabase or new real-Gemini claim is made for this merge.

This handoff pushes code to GitHub main. The existing Vercel production release remains the verified API 2 language build unless separately deployed. Before deploying main, configure and verify its chosen catalog/persistence modes; `CATALOG_SOURCE=bundled ORDER_PERSISTENCE=off` provides the explicit database-free mode. Retained Supabase code requires its existing environment settings. No catalog seed or production environment change was performed for this merge.
...
## Persistent meal and dietary requirements — September 12, 12:54 p.m. EDT

This whole-repository milestone was explicitly authorized after menu consolidation. The eleven campus venues and sourced prices remain; a separate fictional Demo Counter supplies the complete recipe/preparation fixture requested for this demonstration. API remains 2; the menu version changes to `cmu-meal-2026-09-12` because supported Demo configurations and interpretation context changed. Earlier logs are rejected rather than reinterpreted/repriced. `PUBLIC_LOCATION_IDS` contains the eleven active campus IDs plus `demo`; other archived venues remain blocked from the public controller, HTTP context and returned operations.

The order/protocol schema source remains `src/contracts/index.ts`. New exports include `MealRequirements`, `DietaryProfile`, `RequirementsState`, `RequirementDecision`, `RequirementChange`, `CompatibilityResult` and their strict schemas. Catalog-only `FoodEvidence` and its schema are defined in `src/contracts/food.ts`. `src/contracts/requirements.ts` reexports shared definitions. The parse union adds `requirements` (explicit structured changes, optionally item operations) and `decide_requirements` (current decision/choice IDs). Request context may carry accepted requirements and the current proposal; Gemini supplies neither prices nor evidence. Required UI actions are `REQUIREMENTS`, revision-bound `DECIDE_REQUIREMENTS`, and controller-only `RESUME_REQUIREMENTS_DECISION` after abandoned input. New codes are `REQUIREMENT_CONFLICT`, `STAFF_REVIEW_REQUIRED`, and `STALE_DECISION`.

`solveMeal` in `src/core/meal.ts` exhaustively enumerates the selected counter's supported main/side/drink configurations. It honors hard restrictions and locked configurations, then ranks preservation, cents and stable IDs. `checkCompatibility` in `src/core/compatibility.ts` is shared by menu filtering, solver, atomic edits, swaps and review. `src/contracts/food.ts` supplies explicitly fictional Demo recipes and unknown real-campus ingredients/preparation, with provenance and verification dates. Preferences, dislikes and allergies are separate. Unknown/custom or unresolved allergens remain stored and prevent recommendations; there is no allergy override. An explicit preference exception is scoped to the exact configuration and current named preference/dislikes.

The engine keeps accepted meal/cart separate from unaccepted proposals. Declaring a restriction rechecks the existing cart without deleting it and blocks unresolved review/confirmation. Snapshots restore the cart and meal; ordinary Undo preserves the current dietary profile. New order clears profile/conversation. Input start hides/invalidate decisions and preserves only an internal spoken-answer continuation; abandoned input creates fresh revision-bound controls. Draft budget/allergy/dislike fields own capture until applied/discarded, so asynchronous input cannot reuse confirmation. Requirements actions and policy replay deterministically in a detached engine without API/ASR.

Executed candidate evidence: typecheck, lint, build and **697 default tests passed / 29 opt-in checks skipped**. The full production browser suite passed 30 checks; the final runtime corrections passed seven relevant checks, then the newly added browser-offline manual meal/profile → review → receipt check passed with zero interpret requests. CI will run the resulting 31 browser cases. Existing 1,000 generated sequences up to 50 events retain seed `20260912`; this is not formal proof or a new dietary property benchmark. Detailed requirement-to-test mapping: [meal-goal.md](meal-goal.md).

Actual Gemini evidence is distinct from fixture speech/HTTP: nine varied synthetic direct-adapter cases passed, followed by seven real requests and nine browser checkpoints on the final local production server. All returned Gemini with no fallback/retry; the browser reported zero page errors. The defining $12 → proposed $14 → explicitly approved $14 → reviewed simulated receipt completed, as did known dietary/allergy conflicts and persistent unknown-allergen staff review. One earlier harness run failed its text assertion and remains recorded as a failure; the corrected final harness passed. No new human microphone result is inferred. Run `npm run test:meal:live` deliberately; `MEAL_APP_URL` selects the prepared local server or owned public deployment. Reports contain synthetic case IDs, status/mode and pass/fail counts, not transcripts, profile values or raw model output.

Routine API telemetry no longer includes caller-controlled request IDs, transcripts or profiles. Explicit local audit downloads still contain requirements; the engineering panel warns against accidental sharing. No data is automatically sent to staff. No new dependency, API route, service, payment, or deployment stack was added. Existing verified `gemini-3.6-flash` remains server-only. Local manual controls and narrow rules remain available offline.

Publication and fresh public verification follow this candidate record; do not confuse local evidence with a deployed result. Exact script/recovery: [meal-demo.md](meal-demo.md). Remaining limitations are unknown campus food/preparation evidence, missing complete prices/configurations for disabled entries, uncalculated taxes/fees and unverified recognition for this new spoken script. No implementation blocker remains in local checks.

## New kiosk UI integration — September 12, 1:29 p.m. EDT

Started from teammate merge `bd87282a15c86e5d3dd8d5171779450eb7ca7311`, preserving the new category rail, card grid, order rail and flat visual design. Existing meal/dietary controls now use the same visual system: responsive allergy/dislike forms, editable requirement tags, budget/component controls, locks, staff summary and compact cart summary. The single current decision appears above settings, with itemized unapplied proposal and calculated difference beside the accepted cart. Draft ownership and revision-bound actions are unchanged.

Category tabs now scope compatibility/exclusion counts to the displayed category, and conflict explanations are fully readable instead of sharing the card description's two-line clamp. The microphone retains its persistent button and now visibly says **Stop — I’m done** during capture. The Submit button handles declarations and corrections as well as additions. Receipt language explicitly describes simulation; invented pickup/readiness wording was removed, the whole-cart wait is no longer attributed to the currently selected vendor, and receipt rows retain sample/published price provenance. Menu-subtotal labels state that tax/fees are not calculated.

No menu, shared protocol, controller, pure engine, parser or voice implementation changed. Typecheck, lint and production build passed; **698 default tests passed / 29 opt-in checks skipped**, and the full production browser suite passed **32/32** with no retries or skips. This includes invoked HTTP 503 recovery, offline meal controls, ordinary campus ordering, review invalidation, Undo/confirmation, and a new 390px category/dietary check proving full explanation visibility and unchanged accepted cart/profile until an explicit choice. Existing generated engine evidence remains 1,000 sequences up to 50 events with seed `20260912`.
...
Merged with the meal and dietary requirements release the same afternoon: the requirements schemas, engine logic, parser rules, `RequirementsPanel` and the redesigned kiosk were ported to the runtime catalog (`indexCatalog`, `catalogGuard`); `PUBLIC_LOCATION_IDS` became `menu.publicLocationIds` (the ranked shortlist plus the fictional `demo` counter); the merged bundled catalog carries the meal release's version `cmu-meal-2026-09-12`, published to the project as a second immutable version. Merged-tree checks on 2026-09-12 (branch `feat/supabase-persistence` after merging origin/main `bd87282`): `tsc --noEmit` 0 errors; `eslint .` clean; `npm test` 751 passed, 30 env-gated skips; `npm run build` compiled; `npm run check:build` clean (12 client assets); `npm run test:e2e` keyless 33 passed, 3 skipped; `npm run test:e2e:live` 35 passed, 1 skipped, teardown deleted the 38 sessions the run created; `npm run test:db:live` passed; `npm run db:verify` confirms the active version `cmu-meal-2026-09-12` with 46 locations, 7 modifiers, 506 items and 50 previews, and zero order rows visible to the publishable key. The previous version `cmu-shortlist-2026-09-12` remains stored and inactive. Deployed health is still unverified (Vercel environment not set from this machine).

## Dietary marks and ingredient modifiers — September 12, 2026, evening EDT

The user asked to mark items for dietary restrictions from their published descriptions and ingredients, to offer ingredient modifications based on what the menu says, and to remove venues with no orderable menu. Catalog version `cmu-dietary-2026-09-12` stores `foodEvidence` on every item and an `effect` on every modifier: fictional recipes for the Demo Counter, ingredients inferred from the published name and description for campus items (provenance `inferred_campus`, completeness `partial`), and generated `no_<ingredient>` removals for removable toppings named in the wording. Inference only adds ingredients: a detected meat, fish or allergen conflicts with the matching restriction; the absence of a mark is never treated as evidence, so campus items still read **Needs verification** under an allergy, and only a published “veggie”/“vegan” wording satisfies a preference. The kiosk shows the marks on menu cards and cart lines (`data-testid="dietary-mark"`, kinds `claim-vegan`, `claim-vegetarian`, `meat`, `fish`, `contains`, `unknown`) with the note `inference-note` on menus that carry inferred evidence; removing an ingredient in the cart recomputes the line's marks. The selector now lists eight venues; Au Bon Pain, Capital Grains and Schatz (no complete prices) were removed and remain archived.

Evidence: Checks on 2026-09-12 (evening) for branch `feat/dietary-marks`: migration `food_evidence` applied (`items.food_evidence`, `modifiers.effect`); version `cmu-dietary-2026-09-12` published and active (seed batches of 40 after a gateway timeout at 200), `npm run db:verify` matched the bundled release: 46 locations (8 ranked), 14 modifiers, 506 items, 50 previews, zero order rows visible to the publishable key; `tsc --noEmit` 0 errors; `eslint .` clean; `npm test` 764 passed, 30 env-gated skips; `npm run build` compiled and `npm run check:build` found no key material in 12 client assets; `npm run test:e2e` keyless 34 passed, 3 skipped; `npm run test:e2e:live` 36 passed, 1 skipped, teardown deleted the 39 sessions the run created; `npm run test:db:live` passed. Inference outcome: 322 campus items carry inferred evidence, 173 have no cues and stay unverified; 7 generated removals (`no_avocado, no_bacon, no_cheese, no_pickles, no_ranch, no_sauce, no_tomato`) across 25 active items.

## Language handoff (API 2 release branch)

`ParseRequest.language?: "en-US" | "es-ES" | "zh-CN"` is optional and defaults to English. Existing English fixtures remain unchanged. The controller exports `language` and `setLanguage(value)`; it owns cancellation and review invalidation. `src/contracts/languages.ts` owns supported locales and deterministic presentation translations. `src/ui/Language.tsx` supplies context without altering engine data. Selected locale reaches Web Speech recognition, matching speech synthesis and the server Gemini prompt. No provider result supplies prices or confirmation. Export/replay retains canonical accepted events; UI locale is presentation state and does not affect replay.

At the original language handoff, this branch stacked on `work/a-item-notes` / b6b996c, the deployed API 2 release. Main integration is recorded above; deployment remains a separate verified step.

Validation: 754 unit/contract tests passed, 29 opt-in network tests skipped; typecheck and lint passed. The production build and all 36 browser tests passed, including both explicit fixture-language journeys and the HTTP 503 fallback. Two separate local real-Gemini language journeys passed with no fallback and no browser errors. Mocked checks cover recognition locale, late capture cancellation, per-language pack availability, matching TTS voice, missing-voice fallback and Chinese IME Enter. Human multilingual microphone validation remains pending.
