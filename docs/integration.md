...
This branch stacks on `work/a-item-notes` / b6b996c, the deployed API 2 release. It deliberately does not overwrite or deploy main's newer API 3/Supabase work, whose production environment is not configured. Integrate that backend separately with its required hosting configuration.
