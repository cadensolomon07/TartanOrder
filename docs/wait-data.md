...
`config/equivalents.json` contains two small, explicit groups. These are potentially comparable purchases, not identical recipes or allergy guarantees. The engine also checks the selected source line, current prices, modifiers, nearby vendor pair, and wait difference. The configured maximum price difference is 100 cents and minimum wait improvement is 5 minutes. Travel time is not included in the seeded wait savings.
...
| La Prima 12 oz latte | Gates: Latte (12 oz), $5.00 | Wean: Latte (12 oz), $5.00 | Same listed drink and size, different pickup counter |
