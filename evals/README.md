...
4. Every other case calls the parser (`parseRules` in-process; the deployed `/api/interpret` over HTTP, body validated with `ParseResponseSchema`), then `reduceEngine(state, {type:"PARSE_RECEIVED", response})`. `parser` and `fallbackReason` are copied from the response envelope, never assumed.
