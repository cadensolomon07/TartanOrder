...
export function RequirementsSummary({ requirements }: { requirements?: RequirementsState }) {
  ...
  return <section className={styles.summaryPanel} data-testid="requirements-summary" aria-label="Requirements for this cart">
    ...
    <div className={styles.summaryChips}>
      <T>{meal && <><span><T>{meal.budgetCents === null ? "No menu budget" : `Menu budget ${formatCents(meal.budgetCents)}`}</T></span><span><T>{COMPONENTS.filter(component => meal.components.includes(component.id)).map(component => component.label).join(" + ")}</T></span><T>{meal.lockedItemIds.map(id => <span key={id}><T>Keep {(menu.item(id)?.label ?? id)}</T></span>)}</T></>}</T>
...
export function RequirementsPanel({ requirements, lines, acceptedTotalCents, locationId, disabled, onAction, onStartDraft, onEndDraft }: {
  ...
  const proposedTotal = decision?.proposedLines.length ? totalCents(decision.proposedLines, menu.catalog) : null;
  ...
  return <section ref={panel} id="food-requirements" className={styles.panel} data-testid="requirements-panel" aria-label="Your food requirements">
    ...
    {decision && <div className={styles.decision} data-testid="requirements-decision" role="group" aria-label="Choose how to continue">
      ...
      {proposedTotal !== null && <div className={styles.proposal} data-testid="proposed-meal"><strong><T>Proposed · not applied</T></strong><ul>{decision.proposedLines.map(line => <li key={line.lineId}><T>{line.qty}</T> × <T>{(menu.item(line.itemId)?.label ?? line.itemId)}{line.modifiers.length ? ` · ${line.modifiers.map(modifier => (menu.modifier(modifier)?.label ?? modifier)).join(", ")}` : ""}</T><ItemNote note={line.note} /></li>)}</ul><p><T>Proposed menu subtotal: </T><strong><T>{formatCents(proposedTotal)}</T></strong>. <T>{proposedTotal === acceptedTotalCents ? "Same subtotal as your accepted cart." : `${formatCents(Math.abs(proposedTotal - acceptedTotalCents))} ${proposedTotal > acceptedTotalCents ? "more" : "less"} than your accepted cart (${formatCents(acceptedTotalCents)}).`}</T></p></div>}
    ...
    <fieldset disabled={disabled} className={styles.controls}>
      ...
      {meal && <div className={styles.meal}>
        ...
        {meal.selections.length > 0 && <div className={styles.selections} aria-label="Requested meal items">{meal.selections.map(selection => <div key={selection.component}>
          <span><T>{meal.lockedItemIds.includes(selection.itemId) ? "Keep " : "Requested: "}{(menu.item(selection.itemId)?.label ?? selection.itemId)}{selection.modifiers.length ? ` · ${selection.modifiers.map(modifier => (menu.modifier(modifier)?.label ?? modifier)).join(", ")}` : ""}</T></span>
    ...
    <T>{issues.length > 0 && <div className={styles.warning} data-testid="cart-requirement-issues"><h3><T>These cart items need attention</T></h3><T>{issues.map(check => {
      ...
      return <p key={check.lineId}><strong><T>{line ? (menu.item(line.itemId)?.label ?? line.itemId) : "Cart item"}</T> · <T>{check.status === "unknown" ? "Needs verification" : "Conflicts with requirements"}</T></strong><br /><T>{check.reasons.join(" ")}</T></p>;
    })}</T></div>}</T>
    {(profile.allergies.length > 0 || staffIssues.length > 0) && <details className={styles.staff} data-testid="staff-summary" open={staffIssues.length > 0}>
      ...
      {lines.map(line => <p key={line.lineId}><T>{line.qty}</T> × <T>{(menu.item(line.itemId)?.label ?? line.itemId)}{line.modifiers.length ? ` · ${line.modifiers.map(modifier => (menu.modifier(modifier)?.label ?? modifier)).join(", ")}` : ""}</T><ItemNote note={line.note} /></p>)}
