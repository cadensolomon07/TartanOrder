...
export function RequirementsSummary({ requirements }: { requirements?: RequirementsState }) {
  ...
  return <section className={styles.summaryPanel} data-testid="requirements-summary" aria-label="Requirements for this cart">
    ...
    <div className={styles.summaryChips}>
      {meal && <><span>{meal.budgetCents === null ? "No menu budget" : `Menu budget ${formatCents(meal.budgetCents)}`}</span><span>{COMPONENTS.filter(component => meal.components.includes(component.id)).map(component => component.label).join(" + ")}</span>{meal.lockedItemIds.map(id => <span key={id}>Keep {menu.item(id)?.label ?? id}</span>)}</>}
...
export function RequirementsPanel({ requirements, lines, acceptedTotalCents, locationId, disabled, onAction, onStartDraft, onEndDraft }: {
  ...
  const { menu } = useCatalog();
  const itemName = (id: string) => menu.item(id)?.label ?? id;
  const modifierNames = (ids: readonly string[]) => ids.map(id => menu.modifier(id as Parameters<typeof menu.modifier>[0])?.label ?? id).join(", ");
  ...
  return <section ref={panel} id="food-requirements" className={styles.panel} data-testid="requirements-panel" aria-label="Your food requirements">
    ...
    {decision && <div className={styles.decision} data-testid="requirements-decision" role="group" aria-label="Choose how to continue">
      ...
      {proposedTotal !== null && <div className={styles.proposal} data-testid="proposed-meal"><strong>Proposed · not applied</strong><ul>{decision.proposedLines.map(line => <li key={line.lineId}>{line.qty} × {itemName(line.itemId)}{line.modifiers.length ? ` · ${modifierNames(line.modifiers)}` : ""}</li>)}</ul><p>Proposed menu subtotal: <strong>{formatCents(proposedTotal)}</strong>. {proposedTotal === acceptedTotalCents ? "Same subtotal as your accepted cart." : `${formatCents(Math.abs(proposedTotal - acceptedTotalCents))} ${proposedTotal > acceptedTotalCents ? "more" : "less"} than your accepted cart (${formatCents(acceptedTotalCents)}).`}</p></div>}
    ...
    <fieldset disabled={disabled} className={styles.controls}>
      ...
      {meal && <div className={styles.meal}>
        ...
        {meal.selections.length > 0 && <div className={styles.selections} aria-label="Requested meal items">{meal.selections.map(selection => <div key={selection.component}>
          <span>{meal.lockedItemIds.includes(selection.itemId) ? "Keep " : "Requested: "}{itemName(selection.itemId)}{selection.modifiers.length ? ` · ${modifierNames(selection.modifiers)}` : ""}</span>
      ...
      {profile.exceptions.length > 0 && <div className={styles.warning} aria-label="Explicit preference exceptions">{profile.exceptions.map(exception => <p key={`${exception.itemId}-${exception.modifiers.join()}`}>Exception: {itemName(exception.itemId)}{exception.modifiers.length ? ` with ${modifierNames(exception.modifiers)}` : ""}. Allergies still apply. <button disabled={drafting} type="button" onClick={() => change([{ type: "REMOVE_EXCEPTION", itemId: exception.itemId }])}>Remove exception</button></p>)}</div>}
    </fieldset>
    ...
    {issues.length > 0 && <div className={styles.warning} data-testid="cart-requirement-issues"><h3>These cart items need attention</h3>{issues.map(check => {
      ...
      return <p key={check.lineId}><strong>{line ? itemName(line.itemId) : "Cart item"} · {check.status === "unknown" ? "Needs verification" : "Conflicts with requirements"}</strong><br />{check.reasons.join(" ")}</p>;
    })}</div>}
    {(profile.allergies.length > 0 || staffIssues.length > 0) && <details className={styles.staff} data-testid="staff-summary" open={staffIssues.length > 0}>
      ...
      {lines.map(line => <p key={line.lineId}>{line.qty} × {itemName(line.itemId)}{line.modifiers.length ? ` · ${modifierNames(line.modifiers)}` : ""}</p>)}
