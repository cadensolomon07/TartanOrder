...
export function MenuButtons({ disabled, onOps, locationId = "demo", filter = "all" }: { disabled: boolean; onOps: (ops: Op[]) => void; locationId?: LocationId; filter?: MenuFilter }) {
  ...
  return (
    <section className={styles.menu} aria-label="Menu">
      ...
      {ids.length === 0 && previews.length === 0 && <p className={styles.muted}>{query ? "No matching menu items. Try another name." : "Ordering is unavailable at this location: no complete priced items were published. Choose another location."}</p>}
      {shown.filter(category => ids.some(id => MENU[id].category === category.id)).map((category) => <div key={category.id} className={styles.menuCategory}>
