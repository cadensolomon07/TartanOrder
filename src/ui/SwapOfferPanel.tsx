...
export function SwapOfferPanel({ offer, source, disabled, onAction }: { offer: SwapOffer; source: WaitView["source"]; disabled: boolean; onAction(action: UiAction): void }) {
  ...
  return <section className={styles.swapOffer} data-testid="swap-offer" aria-label="Faster item option">
    ...
    <p><strong>{offer.quantity}× {label(menu, offer.alternative.itemId)}</strong> at <strong>{vendor}</strong></p>
    <p>Instead of {label(menu, offer.original.itemId)} at {menu.locationName(offer.original.vendorId)}.</p>
    ...
    {offer.retainedModifiers.length > 0 && <p>Keeps: {offer.retainedModifiers.map((id) => modifierLabel(menu, id)).join(", ")}.</p>}
    {offer.removedModifiers.length > 0 && <p><strong>Removes: {offer.removedModifiers.map((id) => modifierLabel(menu, id)).join(", ")}.</strong></p>}
