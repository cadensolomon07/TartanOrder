import type { CompatibilityResult, DietaryProfile, ItemId, ModifierId } from "@/contracts";
import { foodEvidenceFor, normalizeFoodName } from "@/contracts/food";
import { MENU } from "@/contracts/menu";

/** Pure and shared by browsing, edits, suggestions, swaps, review and confirmation. */
export function checkCompatibility(itemId: ItemId, modifiers: readonly ModifierId[], profile: DietaryProfile): CompatibilityResult {
  const evidence = foodEvidenceFor(itemId, modifiers);
  ...
  if (modifiers.some(modifier => !MENU[itemId]?.allowedModifiers.includes(modifier))) problem("This customization is not supported by the selected item.", "conflict");
