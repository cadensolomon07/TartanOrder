...
export function bundledCatalog(): Catalog {
  ...
  const items = [
    ...
    ...CAMPUS_ITEMS.map((item) => ({
      ...
      priceCents: item.priceCents, aliases: [...item.aliases], allowedModifiers: [], sourcePage: item.sourcePage ?? null,
    })),
  ];
  ...
  const modifiers = Object.values(MODIFIERS).map((modifier) => ({ id: modifier.id, label: modifier.label, priceCents: modifier.priceCents }));
  cached = freezeCatalog(CatalogSchema.parse({
    ...
    locations, items, previews, modifiers,
  }));
  return cached;
}
