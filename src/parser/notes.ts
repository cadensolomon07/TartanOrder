...
function configuration(note: string, itemId: ItemId, menu: CatalogIndex): { note: string; modifiers: ModifierId[] } | Rejection {
  ...
  for (const clause of clauses(note)) {
    ...
    if (modifier) {
      if (menu.item(itemId)?.allowedModifiers.includes(modifier)) {
...
function targetLines(target: string, req: ParseRequest, menu: CatalogIndex): Line[] {
  ...
  const exact = cart.filter(line => [(menu.item(line.itemId)?.label ?? line.itemId), ...(menu.item(line.itemId)?.aliases ?? [])].some(alias => key(alias) === name));
  return exact.length ? exact : cart.filter(line => [(menu.item(line.itemId)?.label ?? line.itemId), ...(menu.item(line.itemId)?.aliases ?? [])].some(alias => ` ${key(alias)} `.includes(` ${name} `)));
}

function editOps(intent: NoteIntent, line: Line, menu: CatalogIndex): Op[] | Rejection {
  const ref: Ref = { by: "line", lineId: line.lineId };
  if (intent.mode === "clear") return [{ type: "SET_NOTE", ref, note: "" }];
  const parsed = configuration(intent.note, line.itemId, menu);
...
export function parseNoteText(req: ParseRequest, parseBase: (req: ParseRequest) => ParseResult | null, catalog: Catalog): ParseResult | null {
  ...
  const guarded = (req.locationId ?? "demo") === "demo" ? guardTranscript(req.text) : guardCampusQuantities(req, catalog);
  ...
  if (intent.mode === "add") {
    ...
    const attach = (ops: Op[]): Op[] | Rejection => {
      ...
      const parsed = configuration(intent.note, op.itemId, menu);
      if ("kind" in parsed) return parsed;
  ...
  const lines = targetLines(intent.target, req, menu);
  ...
  for (const [index, line] of lines.entries()) {
    const ops = editOps(intent, line, menu);
    if (!Array.isArray(ops)) return ops;
    choices.push({ id: `note-line-${index + 1}`, label: `${(menu.item(line.itemId)?.label ?? line.itemId)} (cart row ${(req.context?.lines.indexOf(line) ?? 0) + 1})`.slice(0, LIMITS.labelChars), ops });
  }
...
export function modelNoteError(op: Op, req: ParseRequest, catalog: Catalog): string | null {
  const menu = indexCatalog(catalog);
  ...
  if (modifier && items.some(itemId => menu.item(itemId)?.allowedModifiers.includes(modifier))) {
