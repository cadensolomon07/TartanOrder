import type { ParseRequest, ParseResult, RequirementChange } from "@/contracts";
...
export function parseRequirementsText(req: ParseRequest): ParseResult | null {
  ...
  if (req.context?.requirements?.meal) {
    ...
    if (selection) {
      ...
      const items = itemsForLocation(locationId).filter(item => [item.label, ...item.aliases].some(alias => normalize(alias) === query));
      if (items.length === 1) {
        const item = items[0];
        const old = req.context.requirements.meal.selections.find(value => MENU[value.itemId].category === item.category);
  ...
  const aliases = itemsForLocation(locationId).flatMap(item => [item.label, ...item.aliases]).map(normalize).sort((a, b) => b.length - a.length);
