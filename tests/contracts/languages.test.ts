...
describe("language support", () => {
  it("accepts only the three supported languages without changing existing requests", () => {
    ...
    for (const language of ["es-ES", "zh-CN"] as const) {
      ...
      const response = ParseResponseSchema.parse(parseRules(request));
  ...
  it("reads each selected language from the same immutable priced review", () => {
    ...
    expect(reviewToSpeech(review, "es-ES")).toContain("1 Agua");
    expect(reviewToSpeech(review, "zh-CN")).toContain("1 水");
    for (const language of ["es-ES", "zh-CN"] as const) {
      expect(reviewToSpeech(review, language)).toContain("1.50");
      expect(reviewToSpeech(review, language)).toContain("extra ice");
    }
  });
});
