...
describe("persistent requirements controls", () => {
  it("dispatches distinct preference, allergy and dislike changes", () => {
    ...
    render(withCatalog(<RequirementsPanel acceptedTotalCents={1200} locationId="demo" lines={[]} disabled={false} onAction={onAction} />));
  ...
  it("keeps ambiguous nuts visible until an explicit resolution", () => {
    const onAction = vi.fn();
    render(withCatalog(<RequirementsPanel acceptedTotalCents={1200} locationId="demo" requirements={state({ profile: profile({ allergies: ["nuts"] }) })} lines={[]} disabled={false} onAction={onAction} />));
  ...
  it("converts a decimal budget exactly and rejects fractional cents", () => {
    ...
    render(withCatalog(<RequirementsPanel acceptedTotalCents={1200} locationId="demo" requirements={requirements} lines={[]} disabled={false} onAction={onAction} />));
  ...
  it("dispatches a revision-bound decision and exposes blocking cart notes", () => {
    ...
    render(withCatalog(<RequirementsPanel acceptedTotalCents={1200} locationId="demo" requirements={requirements} lines={[{ lineId: "line1", itemId: "burger", qty: 1, modifiers: [] }]} disabled={false} onAction={onAction} />));
...
describe("evidence-based menu inspection", () => {
  it("separates vegan matches from conflicts while allowing an explicit preference choice", () => {
    const onOps = vi.fn();
    render(withCatalog(<MenuButtons locationId="demo" disabled={false} onOps={onOps} profile={profile({ preference: "vegan" })} />));
  ...
  it("blocks allergy conflicts and unknown preparation without hiding the evidence", () => {
    const onOps = vi.fn();
    render(withCatalog(<MenuButtons locationId="demo" disabled={false} onOps={onOps} profile={profile({ allergies: ["peanut"] })} />));
  ...
  it("uses a meal selection callback without dispatching an ordinary ADD", () => {
    ...
    render(withCatalog(<MenuButtons locationId="demo" disabled={false} onOps={onOps} onMealItem={onMealItem} />));
...
describe("requirement drafts with the real controller (no microphone or provider)", () => {
  function mountStore(store: ReturnType<typeof createOrderController>) {
    ...
    return render(withCatalog(<Host />));
  }
  it("invalidates review on the first requirement keystroke and holds capture until discarded", () => {
    const store = createOrderController({ catalog: CATALOG, sessionId: () => "requirement-input-test" });
  ...
  it("discards a budget draft when typing elsewhere, and cannot release the new input owner", () => {
    const store = createOrderController({ catalog: CATALOG, sessionId: () => "input-owner-test" });
