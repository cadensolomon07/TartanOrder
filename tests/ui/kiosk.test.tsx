...
it("keeps English browser recognition while Spanish and Mandarin require audio capture support", () => {
  mount();
  fireEvent.click(screen.getByTestId("talk"));
  expect(FakeRecognition.last!.lang).toBe("en-US");
  fireEvent.change(screen.getByTestId("language-select"), { target: { value: "es-ES" } });
  expect(screen.queryByTestId("talk")).toBeNull(); // jsdom has no MediaRecorder/getUserMedia
  expect(screen.getByTestId("gemini-audio-notice").textContent).toContain("Gemini");
