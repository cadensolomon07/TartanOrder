...
function proposal(request: ParseRequest, ops: Op[] = [burger]): ParseResponse {
  return { v: 2, requestId: request.requestId, baseRevision: request.baseRevision, menuVersion: "cmu-meal-2026-09-12", parser: "rules", fallbackReason: null, result: { kind: "proposal", ops } };
}
...
describe("controller capture and review gates", () => {
  ...
  it("submit ends capture, creates a fresh input revision, and remains busy until parsing settles", async () => {
    ...
    expect(call.request).toMatchObject({ v: 2, menuVersion: "cmu-meal-2026-09-12", text: "fries", source: "voice", asrConfidence: 0.62, baseRevision: inputRevision + 1 });
