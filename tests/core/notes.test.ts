...
import { API_VERSION, OrderViewSchema, type ItemId, type Op, type ParseResponse, type RequirementChange, type UiAction, type WaitEngineConfig } from "@/contracts";
import { createEngine as createCatalogEngine, exportLog, getView, reduceEngine, replayLog as replayCatalogLog, type EngineState } from "@/core/engine";
import { swapCandidates } from "@/core/waits";

import { CATALOG, MENU_VERSION } from "../helpers/catalog";
const createEngine = (id: string, waitConfig?: WaitEngineConfig) => createCatalogEngine(id,{catalog:CATALOG,waitConfig});
const replayLog = (log: string) => replayCatalogLog(log,CATALOG);
...
describe("item notes as reversible staff requests", () => {
  ...
  it("withholds wait swaps for a noted item and rejects the offer invalidated by adding a note", () => {
    ...
    expect(noted.view.swapOffer).toBeNull(); expect(swapCandidates(noted.view.lines, config, [noted.view.lines[0].lineId], CATALOG)).toEqual([]);
