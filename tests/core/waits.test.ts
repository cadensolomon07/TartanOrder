...
function configuration(overrides: Partial<WaitEngineConfig> = {}): WaitEngineConfig {
  ...
  return {
    ...
    groups: [{ id: "fixture-water", itemIds, differences: Object.fromEntries(itemIds.map(id => [id, `${MENU[id].label} at ${MENU[id].locationId}; packaging may differ.`])) }],
    nearbyPairs: [
...
function onlyPair(original: ItemId, alternative: ItemId, originalWait = 20, alternativeWait = 10): WaitEngineConfig {
  const first = MENU[original].locationId as LocationId;
  const second = MENU[alternative].locationId as LocationId;
  return configuration({
    snapshot: { id: "pair-fixture", source: "seeded", asOf, waits: { [first]: originalWait, [second]: alternativeWait } },
    groups: [{ id: "fixture-pair", itemIds: [original, alternative], differences: { [original]: MENU[original].label, [alternative]: MENU[alternative].label } }],
...
describe("deterministic cart waits", () => {
  it("uses the slowest vendor once rather than summing rows or multiplying quantities", () => {
    const view = deriveWaitView([line(india, "one", 3), line(india, "two", 2), line(millies, "three")], configuration());
    ...
    expect(deriveWaitView([], configuration())).toMatchObject({ status: "empty", estimateMinutes: null, lineWaits: {} });
  });
  ...
  it("does not present a complete estimate when even one vendor wait is missing or null", () => {
    for (const waits of [{ "114": 20 }, { "114": 20, "92": null }]) {
      ...
      expect(deriveWaitView([line(india, "known"), line(exchange, "unknown")], config)).toMatchObject({ status: "unavailable", estimateMinutes: null, lineWaits: { known: 20, unknown: null } });
    }
    const unavailable = configuration({ available: false, unavailableReason: "Provider unavailable" });
    expect(deriveWaitView([line(india)], unavailable)).toMatchObject({ status: "unavailable", estimateMinutes: null, lineWaits: { "line-1": null } });
    expect(swapCandidates([line(india)], unavailable, ["line-1"])).toEqual([]);
  });
  ...
  it.each([
    ...
    expect(deriveWaitView([line(india)], config).status).toBe(known ? "known" : "unavailable");
    expect(swapCandidates([line(india)], config, ["line-1"]).length > 0).toBe(known);
  });

  it("keeps legacy engine views/exports unchanged and isolates its optional configuration from callers", () => {
    const legacy = createEngine("legacy");
    ...
    let state = createEngine("snapshot-copy", config);
...
describe("curated swap candidate boundaries", () => {
  it("includes exactly five minutes of item reduction and excludes anything below it", () => {
    expect(swapCandidates([line(india)], onlyPair(india, millies, 20, 15), ["line-1"])).toHaveLength(1);
    expect(swapCandidates([line(india)], onlyPair(india, millies, 20, 15.01), ["line-1"])).toEqual([]);
    expect(swapCandidates([line(india)], onlyPair(india, millies, 10, 20), ["line-1"])).toEqual([]);
  });
  ...
  it("applies the inclusive one-dollar tolerance per unit and discloses the quantity-scaled price change", () => {
    ...
    const candidate = swapCandidates([line(original, "latte", 3)], config, ["latte"])[0];
    ...
    expect(swapCandidates([line(original)], { ...config, priceToleranceCents: 99 }, ["line-1"])).toEqual([]);
    const reverse = swapCandidates([line(alternative, "latte", 3)], onlyPair(alternative, original), ["latte"])[0];
  ...
  it("requires explicit comparable groups, descriptions, nearby pairs, and different vendors", () => {
    expect(swapCandidates([line(india)], configuration({ groups: [] }), ["line-1"])).toEqual([]);
    expect(swapCandidates([line(india)], configuration({ nearbyPairs: [] }), ["line-1"])).toEqual([]);
    const config = onlyPair(india, millies);
    config.groups[0].differences = { [india]: "Original only" };
    expect(swapCandidates([line(india)], config, ["line-1"])).toEqual([]);
    expect(swapCandidates([line("burger")], onlyPair("burger", "chicken_sandwich"), ["line-1"])).toEqual([]);
    expect(swapCandidates([line(india)], configuration(), [])).toEqual([]);
  });
  ...
  it("ranks overall reduction before item reduction, then price and stable item ID", () => {
    ...
    const candidates = swapCandidates([line(india, "india"), line(exchange, "exchange")], config, ["india", "exchange"]);
    ...
    const tied = swapCandidates([line(india)], configuration({ snapshot: { ...configuration().snapshot, waits: { "114": 20, "136": 10, "115": 10, "94": 10 } } }), ["line-1"]);
  ...
  it("keeps maximum-length curated differences within the strict generated offer contract", () => {
    ...
    const state = act(createEngine("bounded-offer", config), { type: "MANUAL", ops: [add(india)] });
  ...
  it("reports only item savings if another counter prevents a complete cart estimate", () => {
    const config = onlyPair(india, millies);
    const candidate = swapCandidates([line(india, "known"), line(exchange, "unknown")], config, ["known"])[0];
  ...
  it("preserves every supported option and never drops requested exclusions", () => {
    ...
    expect(swapCandidates([line("burger", "burger", 1, ["no_onions"])], onlyPair("burger", "cmu_109_cheeseburger"), ["burger"])).toEqual([]);
  });
});
...
describe("revision-bound swap transaction", () => {
  it("accepts atomically with quantity and row identity preserved, discloses removed paid options, then undoes and replays", () => {
    ...
    let state = act(createEngine("swap-accept", config), { type: "MANUAL", ops: [add("burger", 3, ["double"])] });
  ...
  it("offers at most one newly added line and never adds an offer for quantity edits or Undo", () => {
    let state = act(createEngine("new-lines", configuration()), { type: "MANUAL", ops: [add(india), add(india)] });
  ...
  it.each(["input", "decline", "edit", "review"] as const)("dismisses on %s, rejects the old acceptance, and reproduces suppression during replay", (dismissal) => {
    let state = act(createEngine(`dismiss-${dismissal}`, configuration()), { type: "MANUAL", ops: [add(india)] });
  ...
  it("rejects wrong offer IDs/revisions and a repeated acceptance without a second cart change", () => {
    for (const wrong of ["id", "revision"] as const) {
      let state = act(createEngine(`wrong-${wrong}`, configuration()), { type: "MANUAL", ops: [add(india)] });
      const before = getView(state);
    ...
    let state = act(createEngine("repeat", configuration()), { type: "MANUAL", ops: [add(india)] });
    const offer = getView(state).swapOffer!;
  ...
  it("recomputes the offer rather than trusting a changed snapshot, line, or displayed price", () => {
    const state = act(createEngine("revalidate", configuration()), { type: "MANUAL", ops: [add(india)] });
    const offer = getView(state).swapOffer!;
  ...
  it("cannot leak a swap or new item out of a batch whose later operation is invalid", () => {
    let state = act(createEngine("failed-batch", configuration()), { type: "MANUAL", ops: [add("burger")] });
    const before = getView(state).lines;
