...
const selection = (itemId: ItemId, modifiers: Line["modifiers"] = []) => ({ component: MENU[itemId].category, itemId, modifiers });
...
describe("exhaustive meal solver", () => {
  it("calculates the exact $12 locked-side/drink meal and true below-budget minimum", () => {
    ...
    const exact = solveMeal(requested, profile());
    ...
    const low = solveMeal({ ...requested, budgetCents: 1199 }, profile());
    expect(low.kind).toBe("budget"); expect(low.totalCents).toBe(1200); expect(low.summary.minimumCents).toBe(1200);
    expect(solveMeal(meal({ budgetCents: 1099 }), profile()).summary.minimumCents).toBe(1100);
  });

  it("finds the vegan wrap, fries, and water configuration at $12", () => {
    const solved = solveMeal(meal(), profile({ preference: "vegan" }));
    ...
    expect(solved.selections.every(item => checkCompatibility(item.itemId, item.modifiers, profile({ preference: "vegan" })).status === "match")).toBe(true);
  });
  ...
  it("enumerates valid optional modifiers, while preserving an exact locked configuration", () => {
    ...
    const solved = solveMeal(requested, profile({ allergies: ["egg"] }));
    ...
    const locked = solveMeal({ ...requested, lockedItemIds: ["burger"] }, profile({ allergies: ["egg"] }));
  ...
  it("never drops explicit or preserved paid customizations to fit a budget", () => {
    ...
    const solved = solveMeal(meal({ budgetCents: 1300 }), profile(), existing);
    ...
    const explicit = solveMeal(meal({ budgetCents: 1400, selections: [selection("burger", ["double"])] }), profile());
  ...
  it("does not offer a higher budget for unsupported customizations, locks, or unknown evidence", () => {
    for (const requested of [
      ...
      const solved = solveMeal(requested, profile({ preference: "vegan" }));
  ...
  it("checks all eight independent fixture combinations and prioritizes keeping accepted choices", () => {
    ...
    const catalog = ids.map(id => ({ ...MENU[id], allowedModifiers: [] }));
    const solved = solveMeal(meal({ budgetCents: 1300 }), profile(), [], catalog);
    ...
    for (const main of ids.slice(0, 2)) for (const side of ids.slice(2, 4)) for (const drink of ids.slice(4)) {
      const exact = solveMeal(meal({ budgetCents: 2000, selections: [selection(main), selection(side), selection(drink)] }), profile(), [], catalog);
      expect(exact.totalCents).toBe(costs[index++]); expect(exact.summary.checkedCombinations).toBe(1);
    }
    const kept = solveMeal(meal({ budgetCents: 1300 }), profile(), [line("burger"), line("fries"), line("water")], catalog);
  ...
  it("is deterministic, leaves inputs intact, and reports exact cart constraint failures", () => {
    ...
    expect(solveMeal(requested, declared)).toEqual(solveMeal(requested, declared));
    expect({ requested, declared }).toEqual(saved);
    expect(mealCartConflict([line("grilled_cheese"), line("fries"), line("water")], requested)).toBeNull();
    expect(mealCartConflict([{ ...line("grilled_cheese"), qty: 2 }, line("fries"), line("water")], requested)).toContain("exactly one");
  });
});
