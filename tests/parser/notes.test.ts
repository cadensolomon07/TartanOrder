...
import { API_VERSION, LIMITS, type Line, type ParseRequest, type ParseResult } from "@/contracts";
import { CATALOG, MENU_VERSION, MENU } from "../helpers/catalog";
const itemsForLocation = (id: string) => MENU.itemsForLocation(id);
import { parseRules as parseCatalogRules } from "@/parser/rules";
import { buildSystemInstruction, parseGemini } from "@/parser/gemini.server";

const parseRules = (req: ParseRequest) => parseCatalogRules(req,CATALOG);
const request = (text: string, lines: Line[] = [], locationId: ParseRequest["locationId"] = "demo"): ParseRequest => ({
  v: API_VERSION, menuVersion: MENU_VERSION, requestId: "note-request", baseRevision: 1,
...
const model = async (req: ParseRequest, result: unknown) => {
  ...
  const outcome = await parseGemini(req, { apiKey: "synthetic-test-key", model: "gemini-3.6-flash", timeoutMs: 1000, fetchImpl }, CATALOG);
...
describe("Gemini special requests with injected provider responses", () => {
  ...
  it("documents unverified fulfillment and note preservation without a request whitelist", () => {
    const prompt = buildSystemInstruction(undefined,CATALOG);
...
it("attaches and edits a note using a runtime-only catalog item rather than a bundled ID", () => {
  const catalog = structuredClone(CATALOG);
  catalog.versionId = "runtime-notes-test";
  catalog.items = catalog.items.map(item => item.id === "water" ? { ...item, id: "runtime-water", label: "Test Water", aliases: ["test water"], priceCents: 425 } : item);
  const req = { ...request("test water with extra ice"), menuVersion: catalog.versionId };
  expect(parseCatalogRules(req, catalog).result).toEqual({ kind: "proposal", ops: [add("runtime-water", "extra ice")] });
  const edit = { ...request("add note to my test water: lid on side", [line("runtime-water", "extra ice")]), menuVersion: catalog.versionId };
  expect(parseCatalogRules(edit, catalog).result).toEqual({ kind: "proposal", ops: [{ type: "SET_NOTE", ref: { by: "line", lineId: "cart:1" }, note: "extra ice; lid on side" }] });
});
