...
import { buildProviderSchema as buildProviderSchemaFor, buildSystemInstruction as buildSystemInstructionFor, parseGemini as parseGeminiWith, type GeminiConfig } from "@/parser/gemini.server";
import { parseRules as parseRulesWith } from "@/parser/rules";
import { CATALOG, MENU, MENU_VERSION } from "../helpers/catalog";

// The bundled release catalog stands in for the loaded one; every parser call binds it.
const itemsForLocation = (locationId: string) => MENU.itemsForLocation(locationId);
const parseRules = (req: ParseRequest) => parseRulesWith(req, CATALOG);
const parseGemini = (req: ParseRequest, config: GeminiConfig) => parseGeminiWith(req, config, CATALOG);
const buildProviderSchema = (req?: ParseRequest) => buildProviderSchemaFor(req, CATALOG);
const buildSystemInstruction = (req?: ParseRequest) => buildSystemInstructionFor(req, CATALOG);
