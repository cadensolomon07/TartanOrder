...
import { z } from "zod";
import { API_VERSION, DEMO_ITEM_IDS, ParseResponseSchema, type ItemId, type Line, type LocationId, type Op, type ParseRequest, type ParseResult } from "@/contracts";
...
const smash = MENU.item("cmu_188_smash_d_burger")!;
const latte = MENU.item("cmu_115_la-prima-rohr-latte-12-oz")!;
const missing = CATALOG.locations.find((location) => location.id !== "demo" && MENU.itemsForLocation(location.id).length === 0)!;
...
function wireResult(result: ParseResult, req: ParseRequest): unknown {
  const ids = [...new Set([...MENU.itemsForLocation(req.locationId ?? "demo").map(item => item.id), ...(req.context?.lines.map(line => line.itemId) ?? [])])];
...
describe("campus Gemini scope (mocked transport)", () => {
  it("narrows every item enum to the selected menu plus current cart, with a demo default", () => {
    const scoped = itemEnums(buildProviderSchema(request("add a smashd burger", "188", [line]), CATALOG));
    const expected = [...MENU.itemsForLocation("188"), latte].map((_item, index) => `i${index}`);
    expect(scoped.length).toBeGreaterThan(1);
    scoped.forEach((ids) => expect(ids).toEqual(expected));
    itemEnums(buildProviderSchema(undefined, CATALOG)).forEach((ids) => expect(ids).toEqual([...DEMO_ITEM_IDS]));
    expect(JSON.stringify(buildProviderSchema(request("a burger"), CATALOG))).not.toContain('"burger"');
  });
  ...
  it("sends the selected location and cart-only edit context, without supplying menu prices", async () => {
    ...
    await parseGemini(req, { apiKey: "mock-key", model: "gemini-2.5-flash", timeoutMs: 1000, fetchImpl }, CATALOG);
    ...
    expect(instruction).toContain(MENU.locationName("188"));
    expect(instruction).toContain(`i${MENU.itemsForLocation("188").length} | ${latte.label} | from ${MENU.locationName("115")}`);
    expect(JSON.parse(body.contents[0].parts[0].text).context.lines[0].itemId).toBe(`i${MENU.itemsForLocation("188").length}`);
    ...
    expect(itemEnums(body.generationConfig.responseJsonSchema)[0]).toContain(`i${MENU.itemsForLocation("188").length}`);
  });
  ...
  it("rejects an unknown provider symbol before canonical validation", async () => {
    ...
    await expect(parseGemini(req, { apiKey: "mock-key", model: "gemini-3.6-flash", timeoutMs: 1000, fetchImpl }, CATALOG)).rejects.toMatchObject({ code: "INVALID_MODEL_OUTPUT" });
  });

  it("has no ADD branch or invented enum at a location with no verified items", async () => {
    const req = request("a burger", missing.id);
    const schema = buildProviderSchema(req, CATALOG);
    ...
    expect(buildSystemInstruction(req, CATALOG)).toContain("NO ORDERABLE MENU DATA");
...
describe("campus local rules", () => {
  it.each(["a Smash'd Burger", "a smashd burger", "add two smashd burger"])("orders the canonical apostrophe alias: %s", (text) => {
    expect(parseRules(request(text), CATALOG).result).toEqual({ kind: "proposal", ops: [add(smash.id, text.includes("two") ? 2 : 1)] });
  });

  it("orders every complete canonical campus label as one unit", () => {
    for (const item of CATALOG.items.filter((entry) => entry.locationId !== "demo")) {
      const response = parseRules(request(item.label, item.locationId), CATALOG);
  ...
  it("asks about ambiguous size or sauce instead of choosing a first variant", () => {
    for (const req of [request("a latte", "115"), request("chicken wings"), request("3 piece chicken tenders")]) {
      const result = parseRules(req, CATALOG).result;
      expect(result.kind).toBe("clarify");
      if (result.kind === "clarify") expect(result.choices.length).toBeLessThanOrEqual(3);
    }
    const exact = parseRules(request("Latte (12 oz)", "115"), CATALOG).result;
  ...
  it.each([
    ...
    expect(guardCampusQuantities(req, CATALOG)).toBeNull();
    expect(parseRules(req, CATALOG).result).toMatchObject({ kind: "proposal", ops: [{ type: "ADD", qty: text.startsWith("two") ? 2 : 1 }] });
  });
  ...
  it.each(["18,000 smashd burger", "18,000 3 Piece Chicken Tenders - BBQ", "6 Chicken Wings (8 pieces) - BBQ", "-3 Piece Chicken Tenders - BBQ", "zero smashd burger"])("rejects explicit excessive/negative quantity: %s", (text) => {
    expect(parseRules(request(text), CATALOG).result).toMatchObject({ kind: "reject", code: "QUANTITY_LIMIT" });
  });
  ...
  it("keeps explicit quantity rejection after a mocked model attempts to clamp", async () => {
    ...
    expect(guardCampusQuantities(request("a 12 oz latte", "115"), CATALOG)).toBeNull();
    expect(guardCampusQuantities(request("18,000 12 oz latte", "115"), CATALOG)).toMatchObject({ code: "QUANTITY_LIMIT" });
  });

  it("uses current cart context for remove and quantity edits across locations, and supports undo", () => {
    expect(parseRules(request("remove latte 12 oz", "188", [line]), CATALOG).result).toEqual({ kind: "proposal", ops: [{ type: "REMOVE", ref: { by: "item", itemId: latte.id } }] });
    expect(parseRules(request("make latte 12 oz two", "188", [line]), CATALOG).result).toEqual({ kind: "proposal", ops: [{ type: "SET_QTY", ref: { by: "item", itemId: latte.id }, qty: 2 }] });
    expect(parseRules(request("make that two"), CATALOG).result).toEqual({ kind: "proposal", ops: [{ type: "SET_QTY", ref: { by: "last" }, qty: 2 }] });
    expect(parseRules(request("make that 18,000"), CATALOG).result).toMatchObject({ kind: "reject", code: "QUANTITY_LIMIT" });
    expect(parseRules(request("undo", missing.id), CATALOG).result).toEqual({ kind: "proposal", ops: [{ type: "UNDO" }] });
    expect(parseRules(request("remove latte 12 oz"), CATALOG).result).toMatchObject({ kind: "reject", code: "UNKNOWN_REFERENCE" });
  });

  it("honestly rejects unpriced locations, cross-location additions and unsupported compound wording", () => {
    expect(parseRules(request("a burger", missing.id), CATALOG).result).toMatchObject({ kind: "reject", code: "OFF_MENU" });
    expect(parseRules(request("latte 12 oz"), CATALOG).result.kind).toBe("reject");
    expect(parseRules(request("a smashd burger and fresh cut fries"), CATALOG).result.kind).toBe("reject");
    expect(parseRules(request("a smashd burger", "demo"), CATALOG).result.kind).not.toBe("proposal");
  });
  ...
  it("localOnly uses the campus rules without making an HTTP request", async () => {
    ...
    const result = await interpretWith(request("a smashd burger"), { localOnly: true, catalog: CATALOG }, { fetchImpl, timeoutMs: 1000, isOnline: () => true });
