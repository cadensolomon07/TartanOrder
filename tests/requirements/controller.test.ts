...
import { API_VERSION, MENU_VERSION, PUBLIC_LOCATION_IDS, type ParseRequest, type ParseResponse, type RequirementChange } from '@/contracts';
import { createOrderController } from '@/controller/controller';
import { replayLog } from '@/core/engine';

const setup = (interpret?: (req: ParseRequest) => Promise<ParseResponse>) => createOrderController({sessionId:()=> 'requirements-controller',locationId:'demo',allowedLocationIds:PUBLIC_LOCATION_IDS, ...(interpret ? {interpret} : {})});
...
describe('requirement controller lifecycle',()=>{
 ...
 it('reissues a fresh decision after an abandoned draft without reviving stale buttons',()=>{
  ...
  expect(store.getSnapshot().state.totalCents).toBe(1400);expect(replayLog(store.getSnapshot().exportLog())).toEqual(store.getSnapshot().state);store.dispose();
 });
 it('accepts a spoken choice through the original pending decision without authorizing arbitrary totals',async()=>{
  ...
  expect(store.getSnapshot().parser).toBe('fixture');expect(replayLog(store.getSnapshot().exportLog())).toEqual(store.getSnapshot().state);store.dispose();
 });
